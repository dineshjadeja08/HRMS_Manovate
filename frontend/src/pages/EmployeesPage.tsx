...
import type { Employee, Department, Position, EmploymentStatus } from '../types';
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-gray-200 rounded-md text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary-500"
            >
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-white">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-600 uppercase tracking-wider">Employee</th>
                  ...
                  <tr key={employee.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          <div className="text-sm font-medium text-slate-800">{employee.first_name} {employee.last_name}</div>
                          <div className="text-sm text-slate-500">{employee.employee_number}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-700">{employee.email}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-700">
                      {departments.find(d => d.id === employee.department_id)?.name || 'N/A'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-700">
                      {positions.find(p => p.id === employee.position_id)?.title || 'N/A'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-700">{new Date(employee.hire_date).toLocaleDateString()}</td>
                    <td className="px-6 py-4 whitespace-nowrap"><StatusBadge status={employee.employment_status} /></td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <button onClick={() => openEditModal(employee)} className="inline-flex items-center px-3 py-1 bg-primary-500 hover:bg-primary-600 text-white rounded-md transition-colors">
      ...
      {(showCreateModal || showEditModal) && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 overflow-y-auto">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 my-8 p-6">
            <h3 className="text-xl font-bold text-slate-800 mb-6">{showCreateModal ? 'Create New Employee' : 'Edit Employee'}</h3>
            <form onSubmit={showCreateModal ? handleCreateEmployee : handleUpdateEmployee} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" name="employee_number" value={formData.employee_number} onChange={handleInputChange} required disabled={showEditModal}
                    className="w-full px-3 py-2 border border-gray-200 rounded-md text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary-500 disabled:opacity-50" />
                </div>
                ...
                <div>
                  ...
                  <select name="position_id" value={formData.position_id} onChange={handleInputChange} required
                    className="w-full px-3 py-2 border border-gray-200 rounded-md text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary-500">
                ...
                <div>
                  ...
                  <input type="number" name="salary" value={formData.salary} onChange={handleInputChange} required step="0.01" min="0"
                    className="w-full px-3 py-2 border border-gray-200 rounded-md text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary-500" />
                </div>
              ...
              <div className="flex justify-end space-x-3 mt-6">
                <button type="button" onClick={closeModals} disabled={submitting}
                  className="px-4 py-2 bg-white border border-gray-200 hover:bg-gray-50 text-slate-700 rounded-md transition-colors disabled:opacity-50">Cancel</button>
                <PrimaryButton type="submit" disabled={submitting}>{submitting ? 'Saving...' : showCreateModal ? 'Create Employee' : 'Update Employee'}</PrimaryButton>
              </div>
            </form>
          </div>
        </div>
